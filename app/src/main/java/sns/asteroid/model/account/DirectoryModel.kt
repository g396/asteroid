...
import sns.asteroid.api.Directory
...
class DirectoryModel(credential: Credential): AbstractSearchModel<Account>(credential, query = "", offset = 0) {
    override fun search(offset: Int): Result<Account> {
        val response = Directory(credential.instance, credential.accessToken).getDirectory()
